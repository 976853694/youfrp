...
func (ctl *Control) RegisterProxy(pxyMsg *msg.NewProxy) (remoteAddr string, err error) {
	...
	//新增代理检查
	if ctl.serverCfg.John_San_Api != "" && ctl.serverCfg.John_San_Token != "" {
		ok, err := api.CheckProxy(ctl.serverCfg.John_San_Api, ctl.loginMsg.User, pxyConf, pxyMsg, ctl.serverCfg.John_San_Token)
		if err != nil {
			return remoteAddr, err
		}

		if !ok {
			return remoteAddr, fmt.Errorf("John_San invalid proxy configuration")
		}
	}
