...
func (svr *Service) RegisterControl(ctlConn net.Conn, loginMsg *msg.Login, internal bool) error {
	...
	//新增api检测token
	if svr.cfg.John_San_Api != "" && svr.cfg.John_San_Token != "" {
		valid, err := api.CheckToken(svr.cfg.John_San_Api, loginMsg.User, svr.cfg.John_San_Token)
		if err != nil {
			return err
		}
		if !valid {
			return fmt.Errorf("John_San Auth failed")
		}
	}
