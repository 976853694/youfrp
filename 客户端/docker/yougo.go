...
func main() {
	...
	overwrite := flag.Bool("overwrite", true, "是否覆盖已有的frpc.ini文件") // 默认覆盖
	...
	if *mode == "generate" || *mode == "both" {
		// 检查frpc.ini文件是否存在
		mainConfigPath := filepath.Join(configDir, "frpc.ini")
		...
		if mainConfigExists && !*overwrite {
			fmt.Println("检测到frpc.ini已存在，将使用现有配置文件")
		...
		// 生成配置文件
		configFile := filepath.Join(configDir, fmt.Sprintf("frpc_%d.ini", selectedNodeID))
		err = generateConfig(configFile, selectedNode, selectedProxies)
		if err != nil {
			log.Fatalf("生成节点ID=%d的配置文件失败: %v", selectedNodeID, err)
		}
		
		fmt.Printf("已生成配置文件: %s\n", configFile)
		generatedConfigs = append(generatedConfigs, configFile)
...
RunPart:
	...
	if *mode == "run" || *mode == "both" {
		...
		// 检查frpc.ini是否存在
		if _, err := os.Stat(mainConfigPath); err != nil {
			log.Fatalf("无法启动frpc客户端: frpc.ini文件不存在，请先生成配置文件")
		}
		...
		fmt.Printf("%s -c %s\n", config.FrpcPath, filepath.Join(configDir, "frpc.ini"))
		
		// 自动执行
		if *autoRun {
			fmt.Printf("\n正在自动执行: %s -c %s\n", config.FrpcPath, filepath.Join(configDir, "frpc.ini"))
			cmd := exec.Command(config.FrpcPath, "-c", filepath.Join(configDir, "frpc.ini"))
			cmd.Stdout = os.Stdout
			cmd.Stderr = os.Stderr
			err = cmd.Start()
			if err != nil {
...
// 合并多个配置文件到一个主配置文件
func mergeConfigFiles(configFiles []string, outputFile string) error {
	var builder strings.Builder
	commonSectionAdded := false
		...
		for i := 0; i < len(lines); i++ {
			...
			if strings.HasPrefix(line, "[common]") {
				if !commonSectionAdded {
					...
					// 跳过其他文件的[common]部分
					for i < len(lines) && !strings.HasPrefix(lines[i], "[") || strings.HasPrefix(lines[i], "[common]") {
						i++
					}
					i--
				}
	...
	return ioutil.WriteFile(outputFile, []byte(builder.String()), 0644)
}
...
func getProxiesByUsername(db *sql.DB, username string) ([]Proxy, error) {
	...
	query := "SELECT id, username, proxy_name, proxy_type, local_ip, local_port, " + 
			...
			"remote_port, sk, `header_X-From-Where`, status, lastupdate, node " + 
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&p.RemotePort, &p.Sk, &p.HeaderXFromWhere, &p.Status, &p.LastUpdate, &p.Node,
		)
		if err != nil {
...
// 生成frpc配置文件
func generateConfig(configFile string, node Node, proxies []Proxy) error {
	var builder strings.Builder

	// 生成common配置
	builder.WriteString("[common]\n")
	builder.WriteString(fmt.Sprintf("server_addr = %s\n", node.IP))
	builder.WriteString(fmt.Sprintf("server_port = %d\n", node.Port))
	builder.WriteString("tcp_mux = true\n")
	builder.WriteString("protocol = tcp\n")
	builder.WriteString(fmt.Sprintf("user = %s\n", userToken)) // 使用token作为user
	builder.WriteString(fmt.Sprintf("token = %s\n", node.Token))
	builder.WriteString("dns_server = 114.114.114.114\n\n")
	...
	for _, proxy := range proxies {
		...
		builder.WriteString("privilege_mode = true\n")
		builder.WriteString(fmt.Sprintf("type = %s\n", proxy.ProxyType))
		builder.WriteString(fmt.Sprintf("local_ip = %s\n", proxy.LocalIP))
		builder.WriteString(fmt.Sprintf("local_port = %d\n", proxy.LocalPort))
		
		// 根据不同的隧道类型处理特殊字段
		switch strings.ToLower(proxy.ProxyType) {
		case "http", "https":
			// 处理HTTP/HTTPS特有的字段
			if proxy.Domain != "" && proxy.Domain != "[]" {
				...
				if strings.HasPrefix(proxy.Domain, "[") && strings.HasSuffix(proxy.Domain, "]") {
					...
					for _, domain := range domains {
						...
						if domain != "" {
							builder.WriteString(fmt.Sprintf("custom_domains = %s\n", domain))
						}
					}
				} else {
					builder.WriteString(fmt.Sprintf("custom_domains = %s\n", proxy.Domain))
			...
			if proxy.HostHeaderRewrite != "" {
				builder.WriteString(fmt.Sprintf("host_header_rewrite = %s\n", proxy.HostHeaderRewrite))
			}
			...
			// 移除HTTP/HTTPS隧道的远程端口配置
			// 无论端口是否为标准端口，都不生成remote_port配置项
		case "stcp", "xtcp":
			// STCP/XTCP特有的配置
			builder.WriteString(fmt.Sprintf("remote_port = %s\n", proxy.RemotePort))
			...
			if proxy.Sk != "" {
				...
				// 如果没有提供sk，使用默认值
				builder.WriteString("sk = tgx123456\n")
			}
		default:
			// TCP/UDP等其他类型的隧道
			builder.WriteString(fmt.Sprintf("remote_port = %s\n", proxy.RemotePort))
		...
		if useEncryption == "1" || useEncryption == "true" || useEncryption == "on" {
			builder.WriteString("use_encryption = true\n")
		} else {
			builder.WriteString("use_encryption = false\n")
		}
		...
		if useCompression == "1" || useCompression == "true" || useCompression == "on" {
			builder.WriteString("use_compression = true\n")
		} else {
			builder.WriteString("use_compression = false\n")
		}
		builder.WriteString("\n")
	}
