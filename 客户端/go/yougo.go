...
func main() {
	...
	if *mode == "generate" || *mode == "both" {
		...
		// 生成配置文件
		configFile := fmt.Sprintf("frpc_%d.ini", selectedNodeID)
		...
		err = ioutil.WriteFile("frpc.ini", configData, 0644)
		if err != nil {
...
RunPart:
	...
	if *mode == "run" || *mode == "both" {
		...
		cmd := exec.Command(config.FrpcPath, "-c", "frpc.ini")
