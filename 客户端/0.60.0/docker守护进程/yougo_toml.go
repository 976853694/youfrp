...
func main() {
	...
	frpcPath := flag.String("frpc", "frpc", "frpc可执行文件路径")
	checkInterval := flag.Int("interval", 60, "检查配置变化的间隔（秒）")
	flag.Parse()

	// 从环境变量读取配置（Docker模式）
	envToken := os.Getenv("FRP_TOKEN")
	envNode := os.Getenv("FRP_NODE")
	envInterval := os.Getenv("CHECK_INTERVAL")

	// 优先使用环境变量
	if envToken != "" && *tokenFlag == "" {
		*tokenFlag = envToken
	}
	if envNode != "" && *nodeFlag == "" {
		*nodeFlag = envNode
	...
	if *tokenFlag == "" {
		...
		if err == nil && len(savedToken) > 0 {
			...
			log.Fatalf("未提供访问密钥！请设置环境变量 FRP_TOKEN 或使用 -token 参数")
		}
	} else {
